...
- Name it `<your-github-username>.github.io` → the site will publish at the root, e.g. `https://yourname.github.io/`. With this option, leave `baseurl: ""` in `_config.yml` exactly as it is now.
- Name it anything else, e.g. `the-fracturing-guide` → the site will publish at `https://yourname.github.io/the-fracturing-guide/`. With this option, **edit `_config.yml`** and set:
  ```yaml
...
**3. Enable Pages.** In the new repo on GitHub: **Settings → Pages → Build and deployment → Source: Deploy from a branch → Branch: `main`, folder: `/ (root)`**. Save. GitHub builds the Jekyll site automatically server-side — no Actions workflow, no Ruby install, no local build step required. First deploy usually takes a minute or two; check the "Actions" tab on the repo for build status if the site doesn't appear right away.
...
2. Edit any other page if the campaign has revealed something worth adding — a new region, a faction's public reputation shifting, etc. Same rule: only what the *players* would reasonably know.
3. Commit and push:
   ```bash
   git add -A
